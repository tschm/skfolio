...
import skfolio.utils.validation as skfolio_validation
from skfolio.base import validate_asset_panel as canonical_validate_asset_panel
...
DEPRECATION_MESSAGE = (
    ...
    "removed in version 2.0. Import it from skfolio.base instead."
)
...
class TestDeprecatedValidateAssetPanel:
    """Test suite for the deprecated `validate_asset_panel` re-export."""

    def test_explicit_import_warns_and_resolves(self):
        with pytest.warns(FutureWarning, match=re.escape(DEPRECATION_MESSAGE)):
            from skfolio.utils.validation import validate_asset_panel

        assert validate_asset_panel is canonical_validate_asset_panel

    def test_wildcard_import_warns_and_resolves(self):
        namespace = {}

        with pytest.warns(FutureWarning, match=re.escape(DEPRECATION_MESSAGE)):
            exec("from skfolio.utils.validation import *", namespace)

        assert namespace["validate_asset_panel"] is canonical_validate_asset_panel
        assert (
            namespace["validate_cross_sectional_data"] is validate_cross_sectional_data
        )
