...
# `validate_asset_panel` is deprecated and resolved by the module-level
# `__getattr__` below, so wildcard imports keep working until v2.0.
__all__ = ["validate_asset_panel", "validate_cross_sectional_data"]  # noqa: F822
...
# TODO remove deprecated validate_asset_panel re-export in v2.0
def __getattr__(name: str):
    """Resolve the deprecated `validate_asset_panel` re-export.

    `validate_asset_panel` moved to :mod:`skfolio.base`, which cannot be imported at
    module level here because :mod:`skfolio.base` depends on :mod:`skfolio.utils`. The
    import is therefore deferred until the deprecated name is actually accessed.

    .. deprecated::
        `skfolio.utils.validation.validate_asset_panel` is deprecated and will be
        removed in version 2.0. Import it from :mod:`skfolio.base` instead.
    """
    if name == "validate_asset_panel":
        from skfolio.base import validate_asset_panel

        warnings.warn(
            "skfolio.utils.validation.validate_asset_panel is deprecated and will "
            "be removed in version 2.0. Import it from skfolio.base instead.",
            FutureWarning,
            stacklevel=2,
        )
        return validate_asset_panel
